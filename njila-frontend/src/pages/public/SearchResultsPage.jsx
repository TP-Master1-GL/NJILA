...
import {
  ...
  AlertCircle, ArrowLeft, SlidersHorizontal, Bus, Star, MessageCircle
} from "lucide-react";
...
export default function SearchResultsPage() {
  ...
  // Voyage pour lequel on affiche la modal d'avis
  const [avisVoyage, setAvisVoyage] = useState(null);
  // Voyage pour lequel on affiche les avis (drawer inline)
  ...
  const { data: voyagesRaw = [], isLoading, error } = useQuery({
    ...
    queryFn: () =>
      fleetService.rechercherVoyages({
  ...
  // Normalisation
  const voyages = (
    Array.isArray(voyagesRaw) ? voyagesRaw : voyagesRaw?.results || []
  ).map((v) => ({
    ...v,
    id: v.id || v.Id_voyage,
  ...
  const voyagesAvecPlaces = voyages.map((voyage, index) => {
    ...
    const placesLibres =
      avail?.libres ?? avail?.disponibles?.length ?? voyage.placesRestantes ?? 0;
    return { ...voyage, capacite, placesLibres, placesOccupees: capacite - placesLibres };
  });
  ...
  const filtered = voyagesAvecPlaces.filter((v) => {
    if (
      filtres.classe !== "all" &&
      v.typeVoyage !== filtres.classe &&
      v.type_voyage !== filtres.classe
    )
      return false;
  ...
  return (
    <PublicLayout>
      ...
      {avisVoyage && (
        <AvisModal
          voyage={avisVoyage}
          onClose={() => setAvisVoyage(null)}
          onSuccess={() => setAvisVoyage(null)}
        />
      )}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 text-sm font-bold text-gray-900">
                <span>{recherche.origine || "Toutes villes"}</span>
                <ArrowRight className="w-3 h-3 text-gray-400" />
                <span>{recherche.destination || "Toutes villes"}</span>
              </div>
          ...
          {isLoading ? (
            ...
            <div className="grid gap-4">
              {filtered.map((voyage) => {
                ...
                const avisOuvertPourCeVoyage = avisOuvert === voyage.id;
                    ...
                    <div className="p-6">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        ...
                        <div className="flex items-start gap-4">
                          <div className="w-14 h-14 bg-[#135bec]/5 rounded-2xl flex items-center justify-center text-[#135bec] font-black text-sm flex-shrink-0">
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <Badge
                                variant={
                                  voyage.typeVoyage === "VIP" || voyage.type_voyage === "VIP"
                                    ? "primary"
                                    : "gray"
                                }
                              >
                            ...
                            <div className="flex items-center gap-3 mt-3 flex-wrap">
                              <p className="text-xs">
                                <span
                                  className={`font-bold ${
                                    voyage.placesLibres < 10
                                      ? "text-red-600"
                                      : "text-emerald-600"
                                  }`}
                                >
                                  {voyage.placesLibres} place
                                  {voyage.placesLibres > 1 ? "s" : ""} disponible
                                  {voyage.placesLibres > 1 ? "s" : ""}
                                </span>
                              ...
                              <AvisSection voyageId={voyage.id} compact />
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex md:flex-col items-center md:items-end justify-between gap-3 flex-shrink-0">
                          ...
                          <div className="flex gap-2">
                            {/* Bouton Avis — seulement si connecté */}
                            {isAuthenticated && (
                              <button
                                onClick={() =>
                                  setAvisVoyage(avisVoyage?.id === voyage.id ? null : voyage)
                                }
                                title="Laisser un avis"
                                className="p-3 rounded-xl border border-gray-200 text-gray-500 hover:text-amber-500 hover:border-amber-200 hover:bg-amber-50 transition-all"
                              >
                            ...
                            <button
                              onClick={() =>
                                setAvisOuvert(avisOuvertPourCeVoyage ? null : voyage.id)
                              }
                              title="Voir les avis"
                              className={`p-3 rounded-xl border transition-all ${
                                avisOuvertPourCeVoyage
                                  ? "border-[#135bec]/30 bg-[#135bec]/5 text-[#135bec]"
                                  : "border-gray-200 text-gray-500 hover:text-[#135bec] hover:border-[#135bec]/20 hover:bg-[#135bec]/5"
                              }`}
                            >
                    ...
                    {avisOuvertPourCeVoyage && (
                      <div className="px-6 pb-6 border-t border-gray-100 pt-4">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          {isAuthenticated && (
                            <button
                              onClick={() => setAvisVoyage(voyage)}
                              className="flex items-center gap-1.5 text-xs font-bold text-[#135bec] hover:underline"
                            >
                        ...
                        <AvisSection voyageId={voyage.id} />
                      </div>
                    )}
                  </div>
                );
