...
export default function SeatSelectionPage() {
  ...
  const { voyageSelectionne, placesSelectionnees, togglePlace, setPassagers } = useBookingStore();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <style>{`
        ...
        .seat-btn { transition: all .15s ease; }
        .seat-btn:not(:disabled):hover { transform: scale(1.05); }
        .seat-btn.selected { transform: scale(1.07); box-shadow: 0 4px 12px rgba(19,91,236,.35); }
      `}</style>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex gap-8 flex-col lg:flex-row">
                ...
                {Array.from({ length: Math.ceil(seats.length / 4) }, (_, row) => (
                  <div key={row} className="grid grid-cols-5 gap-1 items-center">
                    {[0,1].map(col => {
                      ...
                      return (
                        <button key={seat.id} disabled={seat.occupe}
                          onClick={() => !seat.occupe && togglePlace(seat)}
                          className={`seat-btn w-10 h-10 rounded-lg text-xs font-bold ${
                            seat.occupe ? "bg-gray-200 text-gray-400 cursor-not-allowed" :
                            selected ? "selected bg-primary-600 text-white" :
                            "border-2 border-gray-200 text-gray-600 hover:border-primary-400 bg-white"
                          }`}>
                          {seat.numero}
                        </button>
                      );
                  ...
                  <div key={place.id} className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Input label="Nom complet" placeholder="Jean Dupont"
                        value={passagersForm[idx]?.nom || ""} onChange={e => handlePassagerChange(idx, "nom", e.target.value)} />
                      <Input label="N° CNI / Passeport" placeholder="Numéro d'identité"
                        value={passagersForm[idx]?.cni || ""} onChange={e => handlePassagerChange(idx, "cni", e.target.value)} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* Récap sticky */}
        <div className="fade-in w-full lg:w-80 flex-shrink-0" style={{ animationDelay: ".1s" }}>
          <div className="bg-primary-600 rounded-xl p-6 text-white sticky top-36">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-primary-100">Places</span>
                <div className="flex gap-1 flex-wrap justify-end">
                  {placesSelectionnees.map(p => (
                    <span key={p.id} className="bg-white/20 px-2 py-0.5 rounded text-xs">{p.numero}</span>
                  ))}
                  {!placesSelectionnees.length && <span className="text-primary-300 text-xs">Aucune sélection</span>}
                </div>
              </div>
              <div className="flex justify-between">
            ...
            <Button size="full" variant="secondary" className="mt-4"
              disabled={placesSelectionnees.length === 0} onClick={handleContinue}>
