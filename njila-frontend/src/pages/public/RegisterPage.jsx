...
import { z } from "zod";
import { Eye, EyeOff, CheckCircle, Cloud } from "lucide-react";
...
const schema = z.object({
  ...
  surname:  z.string().min(2, "Prénom requis (min 2 caractères)"),
  ...
  const onSubmit = async (data) => {
    ...
    if (!result.success) {
      if (result.errorCode === 409) {
        setError("email", {
          type: "server",
          message: result.message || "Cette adresse email est déjà utilisée.",
        });
      } else {
        setError("root", {
          type: "server",
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-white w-full">
          ...
          <div>
            ...
            <p className="text-blue-100 mb-8 leading-relaxed">
              Gérez vos trajets, réservations et votre flotte en toute simplicité.
              Rejoignez des milliers de professionnels dès aujourd'hui.
            </p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <Input label="Prénom" placeholder="Jean"
                error={errors.surname?.message} {...register("surname")} />
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <input placeholder="6XX XXX XXX"
                  className={`flex-1 px-4 py-2.5 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-[#135bec] focus:border-transparent transition-all ${errors.phone ? "border-red-400" : "border-slate-200"}`}
                  {...register("phone")}
                />
              </div>
            ...
            {/* Bandeau erreur générique (500, 504…) */}
            {errors.root && (
              <div className="px-4 py-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-600 flex items-center gap-2">
