...
export default function PaymentPage() {
  ...
  const { voyageSelectionne, placesSelectionnees, passagers, setReservationEnCours, resetBooking } = useBookingStore();
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-slate-950">
        <div className="bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 px-4 py-4">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-gray-500 dark:text-slate-400 hover:text-gray-800 dark:hover:text-slate-200 transition-colors group">
            ...
            <span className="text-sm font-medium text-gray-700 dark:text-slate-200 hidden sm:block">Paiement sécurisé</span>
          </div>
        </div>
          ...
          <div className="max-w-4xl mx-auto flex items-center justify-center gap-4">
            {[["1", "Sélection"], ["2", "Passager"], ["3", "Paiement"]].map(([n, l], i) => (
              <div key={n} className="flex items-center gap-2">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${n === "3" ? "bg-blue-600 text-white" : "bg-green-500 text-white"}`}>
                  {n === "3" ? "3" : "✓"}
                </div>
                <span className={`text-sm font-medium ${n === "3" ? "text-gray-900 dark:text-slate-100" : "text-gray-400"}`}>{l}</span>
        ...
        <div className="max-w-4xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 p-6">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Finaliser votre réservation</h2>
              ...
              <div className="flex items-center justify-between p-4 bg-blue-50 dark:bg-blue-950 rounded-xl mb-6 border border-blue-100 dark:border-blue-800">
                <span className="text-sm font-semibold text-gray-600 dark:text-gray-300">Montant à payer</span>
                <span className="text-3xl font-bold text-blue-600 dark:text-blue-400">{formatMontant(prixTotal)}</span>
              </div>

              <div className="mb-6 p-4 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-xl">
                <p className="text-sm font-semibold text-green-800 dark:text-green-200 mb-1">Passager principal</p>
                <p className="text-sm text-green-700 dark:text-green-300">
                  {passagerPrincipal.prenom} {passagerPrincipal.nom}
                  {passagerPrincipal.cni && <span className="ml-1 text-green-600 dark:text-green-400">— CNI : {passagerPrincipal.cni}</span>}
                </p>
                {passagers.length > 1 && <p className="text-xs text-green-600 dark:text-green-400 mt-1">+ {passagers.length - 1} autre(s) passager(s)</p>}
              </div>

              <div className="space-y-6">
                <div>
                  <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Sélectionnez votre opérateur Mobile Money</p>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { id: "ORANGE_MONEY", label: "Orange Money", bg: "bg-orange-500", emoji: "🟠" },
                      { id: "MTN_MONEY",    label: "MTN MoMo",     bg: "bg-yellow-400", emoji: "🟡" },
                    ].map((op) => (
                      <button key={op.id} onClick={() => setOperateur(op.id)} disabled={isProcessing}
                        className={`p-4 rounded-xl border-2 flex flex-col items-center gap-2 transition-all disabled:opacity-50 ${
                          operateur === op.id ? "border-blue-600 bg-blue-50 dark:border-blue-500 dark:bg-blue-950" : "border-gray-200 dark:border-slate-700 hover:border-gray-300 dark:hover:border-slate-600"
                        }`}>
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-2xl ${op.bg}`}>{op.emoji}</div>
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{op.label}</span>
                        {operateur === op.id && <span className="text-xs text-blue-600 dark:text-blue-400 font-bold">Sélectionné</span>}
                      </button>
                    ))}
                  </div>
                </div>

                <Input label="Numéro de téléphone Mobile Money" placeholder="+237 6XX XXX XXX" icon={Phone}
                  value={telephone} onChange={(e) => setTelephone(e.target.value)} disabled={isProcessing} />

                <div className="p-3 bg-blue-50 dark:bg-blue-950 rounded-lg border border-blue-200 dark:border-blue-800">
                  <p className="text-xs text-blue-700 dark:text-blue-300">
                    Une demande de confirmation sera envoyée sur votre <strong>{operateur === "ORANGE_MONEY" ? "Orange Money" : "MTN MoMo"}</strong>. Acceptez-la dans les 5 minutes.
                  </p>
                </div>
                ...
                {isProcessing && (
                  <div className="flex items-center gap-3 p-4 bg-blue-50 dark:bg-blue-950 rounded-xl border border-blue-200 dark:border-blue-800">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-blue-800 dark:text-blue-200">Création de votre réservation...</p>
                      <p className="text-xs text-blue-600 dark:text-blue-400 mt-0.5">Ne fermez pas cette page.</p>
                    </div>
                  </div>
                )}

                <Button size="lg" className="w-full" onClick={handleLancerPaiement} loading={isProcessing} disabled={!telephone.trim() || isProcessing}>
                  {isProcessing ? "Traitement en cours..." : `Payer ${formatMontant(prixTotal)}`}
                </Button>
                <p className="text-xs text-gray-400 text-center">PAIEMENT SÉCURISÉ ET CHIFFRÉ</p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 p-6 sticky top-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-col gap-1 p-3 bg-gray-50 dark:bg-slate-800 rounded-lg">
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {placesSelectionnees.map((p) => (
                      <span key={p.id} className="text-xs bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full font-semibold">#{p.numero}</span>
                    ))}
                  </div>
                </div>
                <div className="flex flex-col gap-1 p-3 bg-gray-50 dark:bg-slate-800 rounded-lg">
                  <span className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">Passager</span>
                  <span className="text-sm font-semibold text-gray-900 dark:text-white">{passagerPrincipal.prenom} {passagerPrincipal.nom}</span>
                </div>
                <div className="border-t border-gray-200 dark:border-slate-700 pt-3">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-gray-900 dark:text-white">Total</span>
                    <span className="text-2xl font-bold text-blue-600 dark:text-blue-400">{formatMontant(prixTotal)}</span>
                  </div>
                </div>
                <div className="p-3 bg-blue-50 dark:bg-blue-950 rounded-lg text-xs text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                  Besoin d'aide ? <a href="#" className="font-semibold underline">Contacter le support</a>
                </div>
              </div>
            </div>
          </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-slate-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">En attente de confirmation...</h2>
          <p className="text-gray-500 dark:text-gray-400 mb-8">Veuillez confirmer la transaction sur votre appareil mobile.</p>
          <div className="space-y-3">
            <div className="p-4 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl text-left shadow-sm">
              <p className="text-sm font-bold text-gray-800 dark:text-slate-200 mb-3">{operateur === "ORANGE_MONEY" ? "Orange Money" : "MTN MoMo"}</p>
            ...
            <div className="p-3 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-xl">
              <p className="text-xs text-amber-700 dark:text-amber-300">La demande expire dans 5 minutes. N'éteignez pas votre téléphone.</p>
            </div>
            <button onClick={handleConfirmerPaiement} className="w-full py-3.5 px-4 bg-green-600 hover:bg-green-700 active:bg-green-800 text-white font-bold rounded-xl transition-colors">
              J'ai confirmé — Voir mon billet
            </button>
            <button onClick={() => setEtape("paiement")} className="w-full py-2.5 px-4 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300 font-semibold rounded-xl transition-colors text-sm">
              ← Retour
            </button>
          </div>
        </div>
