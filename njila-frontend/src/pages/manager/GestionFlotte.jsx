/**
 * GestionFlotte.jsx – Manager Global
 * Données réelles via fleetService (GET /api/bus/)
 * Ajout via fleetService.ajouterBus (POST /api/bus/)
 * Changement d'état via fleetService.changerEtatBus (PUT /api/bus/{id}/etat/)
 */
      ...
      {isLoading ? (
        ...
        <Card padding={false}>
          ...
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((bus) => {
                  const cfg = statutConfig[bus.etat] || { label: bus.etat, variant: "gray" };
                  return (
                    <tr key={bus.IdBus} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-5 py-4 text-sm text-slate-600">{bus.modele || "—"}</td>
          ...
          <div className="px-5 py-3 flex items-center justify-between border-t border-slate-100 text-sm text-slate-400">
            <span>Affichage de {filtered.length} sur {busList.length} bus</span>
          </div>
        </Card>
      )}
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className="block text-sm font-semibold text-slate-700 mb-1.5">État initial</label>
              <select
                value={form.etat}
