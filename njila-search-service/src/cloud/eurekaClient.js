...
function registerToEureka(port) {
    ...
    const appName    = 'NJILA-SEARCH-SERVICE';
    ...
    const client = new Eureka({
        instance: {
            app:        appName,
            instanceId: `${host}:${appName.toLowerCase()}:${port}`,
            hostName:   host,
            ipAddr:     host,
            port:       { '$': port, '@enabled': true },
            vipAddress: appName.toLowerCase(),
            healthCheckUrl: `http://${host}:${port}/api/search/health`,
            statusPageUrl:  `http://${host}:${port}/api/search/health`,
    ...
    client.start(error => {
        if (error) {
            ...
            process.stderr.write('[EUREKA] OK - njila-search-service enregistre\n');
        }
    });
