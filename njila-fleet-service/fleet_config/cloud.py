...
def register_to_eureka(port: int):
    ...
    instance_id = f"{host}:{APP_NAME}:{port}"   # ✅ format correct
    ...
    try:
        eureka_client.init(
            ...
            duration_in_secs         = 90,
            health_check_url         = f"http://{host}:{port}/api/fleet/health",
            status_page_url          = f"http://{host}:{port}/api/fleet/health",
        )
        sys.stderr.write("[EUREKA] OK - njila-fleet-service enregistre\n")
    except Exception as e:
