...
class Agence(models.Model):
    ...
    statut_global = models.CharField(
        max_length=20,
        choices=StatutGlobalAgence.choices,
        default=StatutGlobalAgence.ACTIVE
    )
    logo_image = models.ImageField(
        upload_to='agences/logos/',
        blank=True,
        null=True,
        help_text="Logo de l'agence uploadé localement (format: JPG, PNG, max 2MB)"
    )
    # ✅ Nouveau champ : URL Cloudinary (string, pas de fichier)
    logo_url = models.URLField(
        max_length=500,
        blank=True,
        null=True,
        help_text="URL Cloudinary du logo de l'agence"
    )
    ...
    def get_logo(self):
        """Retourne l'URL du logo (Cloudinary en priorité, sinon fichier local)"""
        if self.logo_url:
...
class Bus(models.Model):
    ...
    immatriculation = models.CharField(
        ...
        validators=[RegexValidator(
            r'^[A-Z0-9\s]+$',
            "L'immatriculation ne doit contenir que des lettres majuscules et des chiffres"
        )]
    )
...
class Guichetier(models.Model):
    ...
    _id_filiale = models.ForeignKey(
        Filiale, on_delete=models.CASCADE,
        related_name='guichetiers', null=True, blank=True
    )
...
class Trajet(models.Model):
    ...
    filiale_depart = models.ForeignKey(
        Filiale, on_delete=models.PROTECT, related_name='trajets_depart'
    )
    filiale_arrive = models.ForeignKey(
        Filiale, on_delete=models.PROTECT, related_name='trajets_arrivee'
    )
...
class Voyage(models.Model):
    ...
    type_voyage = models.CharField(
        max_length=20, choices=TypeVoyage.choices, default=TypeVoyage.STANDARD
    )
    status = models.CharField(
        max_length=20, choices=StatusVoyage.choices, default=StatusVoyage.PROGRAMME
    )
    ...
    id_chauffeur = models.ForeignKey(
        Chauffeur, on_delete=models.SET_NULL, null=True, blank=True, related_name='voyages'
    )
