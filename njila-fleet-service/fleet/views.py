...
class AgenceListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        publish_agence_created(agence)                 # → user-service
        publish_agence_subscription_request(agence)    # → subscribe-service
        publish_agency_updated_for_booking(agence)     # → booking-service (projection)
...
class AgenceDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        agence = serializer.save()
        publish_agence_updated(agence)             # → user-service
        publish_agency_updated_for_booking(agence) # → booking-service (projection)
...
class FilialeListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        publish_filiale_created(filiale)                 # → user-service
        publish_filiale_updated_for_booking(filiale)     # → booking-service (projection)
...
class FilialeDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        filiale = serializer.save()
        publish_filiale_updated(filiale)                 # → user-service
        publish_filiale_updated_for_booking(filiale)     # → booking-service (projection)
...
class BusListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        bus = serializer.save()
        publish_bus_updated_for_booking(bus)   # → booking-service (projection)
...
class BusRetrieveUpdateDeleteView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        bus = serializer.save()
        publish_bus_updated_for_booking(bus)   # → booking-service (projection)
        logger.info(f"Bus mis à jour: {bus.immatriculation}")
...
class BusStatusUpdateView(APIView):
    ...
    def put(self, request, IdBus):
        ...
        publish_bus_status_changed(bus, ancien_etat, nouveau_etat, raison)  # → notification-service
        publish_bus_updated_for_booking(bus)                                 # → booking-service (projection)

        if nouveau_etat == StatusBus.EN_PANNE:
            publish_bus_breakdown(bus, raison)  # → notification-service

        return Response({
            "message": f"État du bus changé de {ancien_etat} à {nouveau_etat}",
            "bus": {"IdBus": bus.IdBus, "immatriculation": bus.immatriculation, "etat": bus.etat}
...
class VoyageListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        publish_bus_updated_for_booking(bus)       # → booking (état bus mis à jour)
        publish_voyage_updated_for_booking(voyage) # → booking (nouveau voyage)
...
class VoyageDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        voyage = serializer.save()
        publish_voyage_updated_for_booking(voyage)  # → booking-service (projection)
    ...
    def perform_destroy(self, instance):
        ...
        bus.save()
        publish_bus_updated_for_booking(bus)        # → booking (bus libéré)
...
class VoyageStatusUpdateView(APIView):
    ...
    def put(self, request, Id_voyage):
        ...
        if nouveau_status == StatusVoyage.ANNULE:
            ...
            voyage.save()
            publish_voyage_cancelled(voyage, motif)          # → booking-service
            if voyage.IdBus:
                bus = voyage.IdBus
                bus.etat = StatusBus.DISPONIBLE
                bus.save()
            ...
            publish_voyage_delayed(voyage, voyage.date_heure_depart)  # → notification-service

        elif nouveau_status == StatusVoyage.EN_COURS:
            publish_voyage_departed(voyage)                  # → notification-service
        ...
        publish_voyage_updated_for_booking(voyage)           # → booking-service
...
class VoyageAssignBusView(APIView):
    ...
    def post(self, request, Id_voyage):
        ...
        if voyage.IdBus:
            ...
            ancien_bus.save()
            publish_bus_updated_for_booking(ancien_bus)   # → booking (ancien bus libéré)
        ...
        publish_bus_updated_for_booking(bus)              # → booking (nouveau bus en voyage)
        publish_voyage_updated_for_booking(voyage)        # → booking (voyage mis à jour)
...
class VoyageAssignChauffeurView(APIView):
    ...
    def post(self, request, Id_voyage):
        ...
        publish_voyage_updated_for_booking(voyage)   # → booking (chauffeur mis à jour)
